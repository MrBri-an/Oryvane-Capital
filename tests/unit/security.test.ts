...
describe("security contracts",()=>{
 ...
 it("isolates admin routes and mandates AAL2",()=>{const auth=source("src/server/admin/authorization.ts");expect(auth).toContain('currentLevel !== "aal2"');expect(source("src/server/admin/access-context.ts")).toContain("get_current_admin_context");expect(source("src/app/(admin)/admin/(protected)/layout.tsx")).toContain("requireAdmin");});
 ...
 it("keeps market figures real and investment confirmation disclosed",()=>{const market=source("src/server/market-data.ts");expect(market).toContain("api.coingecko.com");expect(market).not.toMatch(/fallbackPrice|mockPrice|fakePrice/);expect(source("src/app/(public)/page.tsx")).toContain("MarketHeatmap");expect(source("src/components/investments/investment-request-form.tsx")).toContain("Investment values may rise or fall");});
});
